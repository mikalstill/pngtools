...
static unsigned int
read_sample(const png_byte *data, int bytedepth)
{
  if (bytedepth == 2)
...
static void
write_sample(png_byte *data, int bytedepth, unsigned int value)
{
  if (bytedepth == 2)
    {
      data[0] = (value >> 8) & 0xFF;
...
static int
map_channel(int out_ch, int channels, int targetchannels)
{
  int out_is_alpha = (targetchannels == 2 || targetchannels == 4) && (out_ch == targetchannels - 1);
...
png_byte *
inflateraster(png_byte *input, png_uint_32 width, png_uint_32 height, int bitdepth,
              int targetbitdepth, int channels, int targetchannels)
{
  ...
  if (bitdepth != targetbitdepth)
    {
      scalefactor = (pow(2.0, (double)targetbitdepth) - 1.0) / (pow(2.0, (double)bitdepth) - 1.0);
  ...
  for (pixel = 0; pixel < npixels; pixel++)
    {
      const png_byte *in_pixel = input + pixel * channels * bytedepth;
