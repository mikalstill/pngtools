...
pnginfo_displayfile(const char *filename, int extractBitmap, int displayBitmap, int tiffnames)
{
  ...
  printf("%s%s...\n", filename, tiffnames == pnginfo_true ? " (tiffinfo compatible labels)" : "");
      ...
      if (PNG_INFO_PLTE == png_get_PLTE(png, info, &palette, &num_palette))
        {
          ...
          printf("(%d colours, %d transparent) ", num_palette, num_trans);
        }
      break;
      ...
      if (displayBitmap == pnginfo_true)
        {
          ...
          printf("(Expanded samples result in %d bytes per pixel, %d channels with %d bytes per "
                 "channel)\n\n",
                 png_get_channels(png, info) * bytespersample, png_get_channels(png, info),
                 bytespersample);
                  ...
                  for (scount = 0; scount < png_get_channels(png, info); scount++)
                    {
                      for (bcount = 0; bcount < bytespersample; bcount++)
                        printf("00");
                      if (scount != png_get_channels(png, info) - 1)
                        printf(" ");
                    }
                  printf("] ");
                  runlen++;
